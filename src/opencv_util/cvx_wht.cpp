...
bool CvxWalshHadamardTransform::generateWHFeature(const cv::Mat & image,
                                                   ...
                                                   vector<Eigen::VectorXf> & features)
{
    ...
    for (int i = 0; i<pts.size(); i++) {
        ...
        // extract feature from all channels separately
...
bool CvxWalshHadamardTransform::generateWHFeatureWithoutFirstPattern(const cv::Mat & rgb_image,
                                                            const vector<cv::Point2d> & pts,
                                                            const int patchSize,
                                                            const int kernelNum,
                                                            vector<Eigen::VectorXf> & features)
{
    ...
    CvxWalshHadamardTransform::generateWHFeature(rgb_image, pts, patchSize, kernelNum, wh_features);
